		...
		if(kakaoKey != null) {
			...
			while(cnt <= 10) {
				...
				try {
					...
					data.put("isExist", scrapingMapper.isProjectExist(board_no));
					...
					registAlarm(job_no, local_no, (Integer) data.get("pk"));
				...
				while(cnt < 10) {
					...
					for(Element infoBox : infoBoxes) {
						...
						try {
							...
							int workType_no = document.getElementsByClass("simple-chip").get(0).text().equals("외주(도급)") ? 4 : 2;
							projectBoardVo.setWorkType_no(workType_no);
							...
							String job_no = "";
							Elements categoryWrapper = document.selectFirst(".project-category").select(".category-wrapper");
							for(Element category_e : categoryWrapper) {
								job_no += getJobNo(category_e.text()) + ",";
							}
							if(job_no.length() > 0) {
							...
							if(workType_no == 2 && document.selectFirst(".project-work-condition") != null) {
								Element localBox = document.selectFirst(".work-condition-data");
								if(localBox != null) {
									String extLoc = localBox.text().split(" ")[0];
									Map<String, Object> result = kakao.getLocalInfo(kakaoKey, extLoc);
...
//      ArrayList<Integer> unionMemberList = new ArrayList<>(set);
        ...
        for(int member_no : memberList) {
        	...
        	scrapingMapper.alarmInsert(data);
        }
	}
